...
$allowedIds = array_map(fn($r)=>(int)$r['id'],$allowed->fetchAll());
...
try {
  ...
  if (!$rep) {
    ...
    if ($rep['status'] !== 'Draft') throw new Exception('Report already submitted/locked.');
    $reportId = (int)$rep['id'];
  }

  if ($wasNew) {
    $dt = new DateTime($reportDate);
    $prevShift = ($shift==='AM')?'PM':'AM';
    if ($shift==='AM') $dt->modify('-1 day');
  ...
  foreach ($input['lines'] as $ln) {
    ...
    if (!isset($allowedSet[$pid])) continue; // enforce product access
  ...
  foreach ($input['expenses'] as $e) {
    ...
    if ($label!=='' && $amt>0) { $insExp->execute([$reportId,$label,$amt]); $totalExp += $amt; }
  }
  ...
  foreach ($input['ewallet'] as $w) {
    ...
    if ($ref!=='' && $amt>0) { $insEW->execute([$reportId,$plat,$ref,$amt,$note]); $totalEW += $amt; }
  }
  ...
  foreach ($input['discounts'] as $d) {
    ...
    if ($name!=='' && $idno!=='' && $pid>0 && $qty>0) { $insDisc->execute([$reportId,$name,$idno,$pid,$qty,null,$disc]); $totalDisc += $disc; }
  }

  $pdo->commit();
  echo json_encode(['ok'=>true,'report_id'=>$reportId,'draft_totals'=>[
    'sales_cents'=>$totalSalesCents,'expenses_cents'=>$totalExp,'ewallet_cents'=>$totalEW,'discounts_cents'=>$totalDisc
  ]]);
} catch (Exception $e) {
  $pdo->rollBack(); http_response_code(400); echo json_encode(['error'=>$e->getMessage()]);
}
